...
def setup_vertices():
    ...
    for i in range(1, Xt.shape[0]):
        p0 = np.array([t[i-1], Xt[i-1], Yt[i-1]])
        p1 = np.array([t[i], Xt[i], Yt[i]])
...
def main():
    ...
    global config, ELEVATION, AZIMUTH 

    ini_file = os.path.join(os.path.dirname(__file__), 'config_opengl.ini')
